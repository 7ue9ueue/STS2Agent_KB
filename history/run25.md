...
## Post-Game Log Analysis - 2026-05-17
- Read: `history/run25.md` line by line; `history/run25_strategy.md`; `history/tool_logs/run25.jsonl`; `kb/strategies/pathing.md`; `kb/strategies/elites.md`; `kb/enemies/elites/decimillipede.md`; `kb/enemies/bosses/doormaker.md`; `kb/cards/ironclad/blood_wall.md`; `kb/cards/ironclad/cascade.md`; `kb/cards/ironclad/thrash.md`.
- Lines 14-31: Neow's Torment and early Uppercut were defensible. The small missed optimization is tactical, not strategic: the Floor 2 Toadpoles line probably had a slightly lower-damage branch, but it did not determine the loss.
- Lines 64-83: Prickly Sponge into Steady Battle Trance/Uppercut was strong, but the next Cultists fight exposed the first meaningful gap: once the Unknown rolled a hard combat, the run accepted 21 net HP loss plus Strength Potion spend. Future similar spots should branch-search before lethal if the fight exits below the next elite/rest threshold.
- Lines 115-120: Gremlin Merc was handled well after retries. The note that Fat Gremlin needs spawn-turn damage is reusable and was correctly promoted.
- Lines 173-180: The Two-Tailed Rats save/load happened after lethal, so it could not improve the 24 HP exit. This is the same harness lesson as Floor 9 but with higher stakes: retry before crossing reward boundary when HP/potion preservation matters.
- Lines 198-213: Yummy Cookie over Golden Compass was correct for this run's low Act 2 entry. The later failure was not from refusing Compass; it was from choosing an Act 2 branch that functionally behaved like an elite path without enough HP/potion buffer.
- Lines 227-238: The Floor 21 shop/route pair was the biggest strategic gap. After buying Burning Pact and removing Strike, the run entered three forced Hive hallways before a rest with only Explosive Ampoule at 46/80. Because the chosen branch later forced an elite, this shop should have prioritized survival math: either buy a healing/defensive potion over removal, or take a route that reached rest/shop sooner even if it reduced reward ceiling.
- Lines 247-260: Chompers and Obscura validated the concern: Explosive Ampoule and Weak Potion were spent just to reach the rest at 9 HP. The decisions inside combat were reasonable, but the route had already converted every hallway into an emergency.
- Lines 262-279: Resting to 33 and taking Heart of Iron into Decimillipede was forced by previous decisions, not a sufficient plan. The best branch preserving 6 HP before turn 4 confirms that one Plating potion plus block engine does not meet the Decimillipede gate without simultaneous AoE or a much larger HP cushion.
- Better choices to test in a similar future run: at Floor 21, compare Burning Pact + survival potion against Burning Pact + removal; avoid any branch with three forced Hive hallways plus a forced elite unless current HP is high and a Decimillipede answer already exists; treat post-combat exits below ~30 HP before a forced elite as losing unless the rest/heal reaches a concrete elite threshold.
- KB updates from this review: pathing, elite strategy, Decimillipede, Golden Compass, and Blood Wall/Doormaker-adjacent notes were updated where the lesson was reusable.
