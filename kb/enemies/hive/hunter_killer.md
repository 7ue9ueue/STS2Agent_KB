---
title: Hunter Killer
source: https://slaythespire.wiki.gg/wiki/Slay_the_Spire_2:Hunter_Killer
category: regular
act: hive
stub: false
---

# Hunter Killer

## Overview

**Hunter Killer** is a normal Enemy found in Hive.

## Stats

### Hunter Killer
- **HP**: 121 (Asc8+: 126)
- **Appears in**: Debut Hive

### Attacks

| Name | Intent | Effect |
| --- | --- | --- |
| Tenderizing Goop |  | Applies 1 Tender. |
| Bite |  | Deals 17 damage. |
| _(Asc variant)_ | | Deals 19 (Asc9+) damage. |
| Puncture |  | Deals 7 damage ×3. |
| _(Asc variant)_ | | Deals 8 (Asc9+) damage ×3. |

## Pattern

Always opens with Tenderizing Goop. After that, randomly uses Bite or Puncture, with Puncture being twice as likely. Bite cannot be used twice in a row.

## Update History

_None._

---
<!-- META-AGENT NOTES — do not edit above this line -->
## Strategy Notes

<!-- Reserved for the meta-agent. Append lessons, combos, and heuristics here.
     Wiki facts live above the divider. -->
- Tender punishes Corruption multi-card turns by reducing Strength and Dexterity after each card; front-load high-impact attacks/block before utility cards.
- If the fight is likely to last past the first deck cycle, consider early scaling or a fast kill plan rather than spending many low-impact free skills.
- At very low HP, use one large block card before damage/draw utility; Tender can make later block too small even when a potion or Weak is available.
- Flame Barrier-style thorns are not reduced by Tender and can finish Puncture turns after blocking the first hits.
